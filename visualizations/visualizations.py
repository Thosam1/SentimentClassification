...
def plot_training_history(history):
    ...
    train_acc = history[
        "train_acc"
    ]  # [elt.detach().cpu().numpy() for elt in history['train_acc']]
    val_acc = history[
        "val_acc"
    ]  # [elt.detach().cpu().numpy() for elt in history['val_acc']]
...
def visualise_dataloader(dl, id_to_label=None, with_outputs=True):
    ...
    if with_outputs:
        ...
        print("=============")
        print(
            f"Unique samples seen (approx): {len(set(idxs_seen))}/{total_num_samples}"
        )
        for i in range(3):
            avg = np.array(class_batch_counts[i]).mean()
            print(
                f"Avg samples per batch for class {id_to_label[i] if id_to_label else i}: {avg:.2f}"
            )
        print("=============")
